...
| Bash completion | No documented generated completion command | `braid completion bash` prints a Bash completion script | Source it from shell startup or install it in the Bash completion directory to complete global options, commands, command options, and configured mirror paths. |
